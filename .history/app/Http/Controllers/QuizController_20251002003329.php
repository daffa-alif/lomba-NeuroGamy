...
{
    // contoh ambil dari request
    $bookId = $request->book_id ?? null;
    $book = $bookId ? \App\Models\Book::find($bookId) : null;

    // ambil halaman dibaca (misalnya dari ScoreLogs atau dari request)
    $pagesRead = $request->pages ?? 0;

    return view('Quiz.confirmation', compact('book', 'pagesRead'));
}
