...
use App\Http\Controllers\{AiController, AuthController, BookController, UserController, BookClassificationController, LibraryController, ReadingController};
...
Route::get('/reading/{id}', [ReadingController::class, 'index'])->name('reading.index');
Route::post('/reading/{id}/summarize', [ReadingController::class, 'summarize'])->name('reading.summarize');

Route::get('/reading/{id}', function ($id) {
    $book = \App\Models\Book::findOrFail($id);

    if (!Storage::disk('private')->exists($book->file_name)) {
        abort(404);
    }

    return response()->file(storage_path('app/private/' . $book->file_name));
})->name('reading.file');