...
$(document).ready(function () {
    ...
    $('#btnAddBook').on('click', function () {
        ...
        $('#bookForm')[0].reset();
        $('#book_id').val('');
        $('#bookModal').removeClass('hidden').addClass('flex');
    });

    // CLOSE MODAL
    $('#btnCloseModal').on('click', function () {
        $('#bookModal').addClass('hidden').removeClass('flex');
    });
    ...
    $('#bookForm').on('submit', function (e) {
        ...
        // kalau ada new classification → kirim juga
        if ($('#new_classification').val() !== '') {
            data += '&new_classification=' + $('#new_classification').val();
        }
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success) {
                    location.reload(); // sementara reload, nanti bisa append row tanpa reload
                }
            }
        });
    });
    ...
    $(document).on('click', '.btnEdit', function () {
        ...
        $('#classification_id option').filter(function(){
