    ...
    <div class="bg-white w-full max-w-lg rounded-lg shadow p-6">
        ...
        <form id="bookForm">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="book_title" id="bookTitle" class="w-full border rounded px-3 py-2" required>
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium">Description</label>
                <textarea name="book_description" id="bookDescription" class="w-full border rounded px-3 py-2" required></textarea>
            </div>
...
$(function () {
    ...
    function resetForm() {
        $("#bookForm")[0].reset();
        $("#bookId").val('');
        // remove any _method hidden inputs if present
        $("#bookForm").find('input[name="_method"]').remove();
    }

    // Open modal Add (delegated)
    ...
    $(document).on("submit", "#bookForm", function (e) {
        ...
        // prepare form data and include _method for Laravel if needed
        let formArray = $(this).serializeArray();
        if (method !== "POST") {
            formArray.push({ name: "_method", value: method });
        }

        $.ajax({
            url: url,
            type: "POST", // always POST; use _method for PUT
            data: $.param(formArray),
            success: function (res) {
                // You can update row via JS instead of reload if desired.
                location.reload();
            },
            error: function (xhr) {
                console.error(xhr);
